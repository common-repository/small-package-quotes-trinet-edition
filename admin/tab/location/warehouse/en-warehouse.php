...
namespace EnTrinetWarehouseTemplate;

use EnTrinetWarehouse\EnTrinetWarehouse;

if (!class_exists('EnTrinetWarehouseTemplate')) {

    class EnTrinetWarehouseTemplate
    {
        ...
        {
            $en_heading = $en_data = [];
            self::$en_warehouse_list = EnTrinetWarehouse::get_data(['location' => 'warehouse']);

            // Load function for plans implementation of multi warehouse
            self::en_get_multi_warehouse_plan_status();

            extract(\EnLocation::en_location_filter_data('warehouse'), null);
