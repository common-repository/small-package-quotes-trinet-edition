...
namespace EnTrinetLocationAjax;

use EnTrinetDistance\EnTrinetDistance;
use EnTrinetDropshipTemplate\EnTrinetDropshipTemplate;
use EnTrinetWarehouse\EnTrinetWarehouse;
use EnTrinetWarehouseTemplate\EnTrinetWarehouseTemplate;

if (!class_exists('EnTrinetLocationAjax')) {

    class EnTrinetLocationAjax
    {

        public function __construct()
        {
            add_action('wp_ajax_nopriv_en_trinet_location_save_form_data', [$this, 'en_trinet_location_save_form_data']);
            add_action('wp_ajax_en_trinet_location_save_form_data', [$this, 'en_trinet_location_save_form_data']);

            add_action('wp_ajax_nopriv_en_trinet_get_location', [$this, 'en_trinet_get_location']);
            add_action('wp_ajax_en_trinet_get_location', [$this, 'en_trinet_get_location']);

            add_action('wp_ajax_nopriv_en_trinet_location_delete_row', [$this, 'en_trinet_location_delete_row']);
            add_action('wp_ajax_en_trinet_location_delete_row', [$this, 'en_trinet_location_delete_row']);
        }

        /**
         * Get response from api when we sent zip code
         */
        public function en_trinet_get_location()
        {
            if (isset($_POST['en_trinet_location_zip']) && strlen($_POST['en_trinet_location_zip']) > 4) {
                $get_address = EnTrinetDistance::get_address(sanitize_text_field($_POST['en_trinet_location_zip']), 'address');
            ...
            echo wp_json_encode(['message' => EN_TRINET_709]);
            exit;
        }
         ...
         */
        public function en_trinet_location_delete_row()
        {
            global $wpdb;
            ...
            if ($location === 'warehouse') {
                $en_location_html = EnTrinetWarehouseTemplate::en_load();
                $en_target_location = '.en_location_warehouse_main_div';
            } else if ($location === 'dropship') {
                $en_location_html = EnTrinetDropshipTemplate::en_load();
         ...
         */
        public function en_trinet_location_save_form_data()
        {
            global $wpdb;
            $post_data = [];
            ...
            if (isset($post_data['id'], $post_data['location'])) {
                ...
                if ($location === 'warehouse') {
                    ...
                    $en_location_template_obj = new EnTrinetWarehouseTemplate();
                    $en_target_location = '.en_location_warehouse_main_div';
                    $validate = ['zip', 'city', 'state', 'country', 'location'];
                } else {
                    $location_step = 'Dropship';
                    $en_location_template_obj = new EnTrinetDropshipTemplate();
                    $en_target_location = '.en_location_dropship_main_div';
                ...
                $en_location_data = EnTrinetWarehouse::get_data($en_intersected_data);
