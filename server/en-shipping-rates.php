 ...
 * Class _EnTrinetShippingRates
 */
if (!class_exists('WC_EnTrinetShippingRates')) {

    class WC_EnTrinetShippingRates
    {

        /**
         * Hook for call.
         * _EnTrinetShippingRates constructor.
         */
        public function __construct()
        {
             ...
             */
            add_action('woocommerce_shipping_init', 'en_trinet_shipping_rates');
        }
        ...
        {
            $shipping_rates = new EnTrinetShippingRates();
 ...
 */
if (!function_exists('en_trinet_shipping_rates')) {

    function en_trinet_shipping_rates()
    {

        /**
         * Add class for shipping rates
         */
        class EnTrinetShippingRates extends WC_Shipping_Method
        {
            ...
            /**
             * Hook for call
             * EnTrinetShippingRates constructor.
            ...
            public function __construct($instance_id = 0)
            {
                ...
                $this->method_title = __('Trinet Express');
                $this->method_description = __('Shipping rates from Trinet Express.');
                $this->supports = array(
                    'shipping-zones',
                    'instance-settings',
                ...
                $this->enabled = "yes";
                $this->title = 'Small Package Quotes - Trinet Edition';
                $this->init();
            }
            ...
            public function init_form_fields()
            {
                $this->instance_form_fields = [
                    'enabled' => [
                        'title' => __('Enable / Disable', 'trinet'),
                        'type' => 'checkbox',
                        'label' => __('Enable This Shipping Service', 'trinet'),
                        'default' => 'no',
                        'id' => 'en_trinet_enable_disable_shipping'
                    ]
                ];
            }
            ...
            public function calculate_shipping($package = [])
            {
                ...
                // Eniture Debug Mood
                do_action("eniture_debug_mood", EN_TRINET_NAME . " Plan ", EN_TRINET_PLAN);
                do_action("eniture_debug_mood", EN_TRINET_NAME . " Plan Message ", EN_TRINET_PLAN_MESSAGE);
                ...
                if (empty($en_package)) {
                    $this->en_package = $en_package = \EnTrinetPackage\EnTrinetPackage::en_package_converter($package);
                ...
                if (!empty($this->small_package) && empty($reasons)) {
                    ...
                    // Eniture Debug Mood
                    do_action("eniture_debug_mood", EN_TRINET_NAME . " Package ", $en_package);

                    add_filter('en_eniture_shipment', [$this, 'en_eniture_shipment']);

                    $en_package = array_merge(json_decode(EN_TRINET_GET_CONNECTION_SETTINGS, true), $en_package, json_decode(EN_TRINET_GET_QUOTE_SETTINGS, true));

                    $response = \EnTrinetCurl\EnTrinetCurl::en_trinet_sent_http_request(EN_TRINET_HITTING_API_URL, $en_package, 'POST', 'Quotes');

                    $en_rates = \EnTrinetResponse\EnTrinetResponse::en_rates(json_decode($response, true), $en_package);
                    ...
                    // Eniture Debug Mood
                    do_action("eniture_debug_mood", EN_TRINET_NAME . " Rates ", $en_rates);
                ...
                do_action("eniture_debug_mood", EN_TRINET_NAME . " Total Execution Time ", $en_calculate_shipping_end);
                ...
                if (isset($en_package['shipment_type']) && is_array($en_package['shipment_type'])) {
                    foreach ($en_package['shipment_type'] as $origin_zip => $shipment) {
                        if (isset($shipment['SMALL']) && !isset($shipment['LTL'])) {
                            ...
                            $this->ltl_package[$origin_zip] = EN_TRINET_DECLARED_TRUE;
