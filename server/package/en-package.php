...
namespace EnTrinetPackage;

use EnTrinetDistance\EnTrinetDistance;
use EnTrinetProductDetail\EnTrinetProductDetail;
use EnTrinetReceiverAddress\EnTrinetReceiverAddress;
use EnTrinetWarehouse\EnTrinetWarehouse;

/**
 * Get items detail from added product in cart|checkout page.
 * Class EnTrinetPackage
 * @package EnTrinetPackage
 */
if (!class_exists('EnTrinetPackage')) {
    ...
    class EnTrinetPackage
    {
        ...
        {
            $product_detail_obj = new EnTrinetProductDetail();
            ...
            self::$receiver_address = EnTrinetReceiverAddress::get_address();
            ...
            foreach ($package['contents'] as $key => $product) {
                if (isset($product['data'])) {
                    ...
                    $shipment_type = EN_TRINET_DECLARED_FALSE;
                    $origin_zip_code = EN_TRINET_DECLARED_ZERO;
                    // Micro Warehouse
                    ...
                    foreach ($en_product_fields as $en_field_key => $en_custom_product) {
                        ...
                        if (is_callable(array(__CLASS__, $en_function_trigger)) && strlen($is_line_item) > EN_TRINET_DECLARED_ZERO) {
                            ...
                            $suscription_and_features = apply_filters(
                                "trinet_plans_suscription_and_features", $is_plans
                            );
                            ...
                            if (!is_array($en_location_value) && strlen($en_location_value) > EN_TRINET_DECLARED_ZERO) {
                            ...
                            if (isset($en_location_value['senderZip']) &&
                                ...
                                $origin_zip_code = $en_location_value['senderLocation'] . '_' . $en_location_value['senderZip'];
                                ...
                                switch (EN_TRINET_DECLARED_TRUE) {
                                    case $shipping_class == 'ltl_freight':
                                        $shipment_type = EN_TRINET_DECLARED_TRUE;
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        self::$en_request['LTL_FREIGHT'] = EN_TRINET_DECLARED_ONE;
                                        break;
                                    case $shipment_weight > EN_TRINET_SHIPMENT_WEIGHT_EXCEEDS_PRICE &&
                                        EN_TRINET_SHIPMENT_WEIGHT_EXCEEDS == 'yes':
                                        self::en_set_ltl_shipment($origin_zip_code);
                                        $shipment_type = EN_TRINET_DECLARED_TRUE;
                                        $product_weight < EN_TRINET_SHIPMENT_WEIGHT_EXCEEDS_PRICE ?
                                            self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_TRINET_DECLARED_TRUE : "";
                                        break;
                                    case $product_weight < EN_TRINET_SHIPMENT_WEIGHT_EXCEEDS_PRICE:
                                        $shipment_type = EN_TRINET_DECLARED_TRUE;
        ...
        {
            $reasons = !self::$shipment_type ? array_merge($reasons, [EN_TRINET_712]) : $reasons;
            return (!self::$origin_zip_code > 0) ? array_merge($reasons, [EN_TRINET_713]) : $reasons;
        }
        ...
        static public function en_filter_shipment()
        {
            ...
            self::$en_request = apply_filters("en_woo_addons_carrier_service_quotes_request", self::$en_request, EN_TRINET_SHIPPING_NAME);
            ...
            self::en_trinet_quote_settings_detail();
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['LTL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['LTL'] = EN_TRINET_DECLARED_TRUE;
        }
        ...
        {
            self::$en_request['shipment_type']['LTL_FREIGHT'][$origin_zip_code]['SMALL'] = self::$en_request['shipment_type']['LTL_SMALL'][$origin_zip_code]['SMALL'] = EN_TRINET_DECLARED_TRUE;
        }
         ...
         * @return array|false
         */
        static public function en_trinet_sender_location_address($location)
        {
        ...
        static public function en_is_instore_pickup_enabled($location, $sender_location_address)
        {
            ...
            $suscription_and_features = apply_filters(
                "trinet_plans_suscription_and_features", 'instore_pickup_local_delivery'
            );

            if (!is_array($suscription_and_features)) {
                extract($location, null);
         ...
         * @return array|false
         */
        static public function en_trinet_receiver_location_address($receiver_address)
        {
         ...
         * @return array|false
         */
        static public function en_trinet_quote_settings_detail()
        {
            $delivery_estimate_option = $cutt_off_time = $fulfilment_offset_days = '';
            $settings = $shipment_days = [];
            $feature_option = apply_filters("trinet_plans_suscription_and_features", 'cutt_off_time');
            $en_settings = json_decode(EN_TRINET_SET_QUOTE_SETTINGS, true);
            extract($en_settings, null);
         ...
         */
        static public function en_trinet_get_location($location_id = '')
        {
            ...
            $en_location = EnTrinetWarehouse::get_data($en_where_clause);
            // Micro Warehouse
            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
            ...
            if (!empty($en_location) && is_array($en_location)) {
                ...
                if (count($en_location) == 1) {
                    return self::en_trinet_sender_location_address(reset($en_location));
                } else {
                    ...
                    $receiver_address = self::en_trinet_receiver_location_address($receiver_address);
                    $get_address = json_decode(
                        EnTrinetDistance::get_address($en_location, $en_access_level, $receiver_address), true);
                    return (isset($get_address['origin_with_min_dist']) && !empty($get_address['origin_with_min_dist'])) ?
                        self::en_trinet_sender_location_address($get_address['origin_with_min_dist']) : [];
                }
            }
                    ...
                    switch ($enable_dropship) {
                        case 'yes':
                            ...
                            if (stripos(implode($all_plugins), 'micro-warehouse-shipping.php')  || is_plugin_active_for_network('micro-warehouse-shipping-for-woocommerce/micro-warehouse-shipping.php')) {
                                $locations_dropship = maybe_unserialize(get_post_meta(self::$post_id, '_dropship_location', true));
                                if(!empty($locations_dropship)) {
                                    self::$get_minimum_warehouse = self::en_trinet_get_location();
                                    return self::$get_minimum_warehouse;
                                }
                            }
                            return self::en_trinet_get_location(get_post_meta(self::$post_id, self::$product_key_name, true));
                        default:
                            self::$get_minimum_warehouse = self::en_trinet_get_location();
                            return self::$get_minimum_warehouse;
                    }
